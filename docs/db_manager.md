...
- **스마트 초기화**: 시작 시 테이블 존재 확인 및 자동 생성
- **안정적 에러 처리**: 테이블 관련 에러 발생 시 프로그램 안전 종료
...
- `_initialize_tables()`: 초기화 시 자동 테이블 생성 (내부 메서드)
...
**새로운 아키텍처**: DatabaseManager 초기화 시 자동으로 테이블을 확인하고 생성합니다.

### 작동 원리

1. **DatabaseManager 생성 시**:
   - 데이터베이스 연결 테스트
   - 모든 필요한 테이블 확인 및 생성
   - 실패 시 프로그램 종료 (`sys.exit(1)`)

2. **데이터 저장 중**:
   - 테이블이 존재한다고 가정하고 바로 저장 시도
   - 테이블 관련 에러 발생 시 프로그램 즉시 종료
   - 매번 테이블 존재 여부를 확인하지 않아 성능 향상

### 장점

- **성능**: 매 저장 시마다 테이블 존재 확인하지 않음
- **안정성**: 초기화 시 모든 테이블을 확실히 생성
- **단순성**: 저장 로직이 단순하고 명확
- **안전성**: 테이블 문제 시 즉시 프로그램 종료로 데이터 무결성 보장

### 사용 예시

```python
# 초기화 시 자동으로 모든 테이블 생성됨
db_manager = DatabaseManager()  # 여기서 테이블이 생성되거나 프로그램 종료

# 이후 저장은 단순하게
record = TradingHistoryRecord(...)
success = db_manager.save_trading_history(record)  # 테이블 존재 체크 없이 바로 저장
```
...
# 데이터베이스 매니저 초기화 - 자동으로 테이블 생성됨
# 실패 시 프로그램 자동 종료
db_manager = DatabaseManager()

print("데이터베이스 초기화 완료 - 모든 테이블 준비됨")
```
...
# 데이터베이스에 저장 (테이블은 이미 존재함)
if db_manager.save_trading_history(record):
    print("거래 기록 저장 완료")
# 만약 테이블 관련 에러가 발생하면 프로그램이 자동 종료됨
```
...
1. **초기화 중요성**: DatabaseManager 생성 시 테이블 초기화가 실패하면 프로그램이 종료됩니다
2. **보안**: 데이터베이스 비밀번호는 환경 변수나 안전한 설정 파일에 저장하세요
3. **연결 관리**: `get_connection()` 컨텍스트 매니저를 사용하여 안전한 연결 관리를 하세요
4. **에러 처리**: 테이블 관련 에러 발생 시 프로그램이 자동 종료되므로 로그를 확인하세요
5. **성능**: 초기화 후에는 매번 테이블 존재 확인을 하지 않아 빠른 성능을 보장합니다
6. **권한**: PostgreSQL 사용자에게 데이터베이스 및 테이블 생성 권한이 있어야 합니다

## 문제 해결

### 초기화 오류
- DatabaseManager 생성 시 프로그램이 종료되는 경우 로그를 확인하세요
- PostgreSQL 서비스가 실행 중인지 확인
- 데이터베이스 연결 설정 확인
- 사용자 권한 확인

### 런타임 테이블 오류
- 프로그램 실행 중 갑작스런 종료 시 테이블 손상 가능성 확인
- 데이터베이스 로그 확인
- 수동으로 `create_tables()` 호출하여 테이블 복구