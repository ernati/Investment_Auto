...
- 기본 namespace: `investment-auto`
- 기본 label selector: `app.kubernetes.io/name=investment-auto` (Helm `selectorLabels` 헬퍼가 생성하는 label 기준)
...
CronJob의 ServiceAccount는 Pod 로그 조회(`pods/log`)에 대한 `get` 권한만 가져야 하며, Pod/Deployment/Service 등 다른 리소스의 수정 권한을 가져서는 안 된다. ServiceAccount, ClusterRoleBinding, Secret, CronJob 모든 manifest는 `investment-auto` namespace에 배포된다.
...
- 의존성(`requirements.txt` 전체)은 `/app/libs`에 설치되고, 메인 컨테이너의 `PYTHONPATH=/app/libs:/app/Scripts`로 인식된다.
